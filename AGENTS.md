...
- Предпочитается RAII (например, `MutexGuard`, `CriticalSection`).
...
- **ISR-безопасность:** `paraos::ISRbool` и `VarAtomic` спроектированы для контекстов прерываний/сервисных вызовов; где уместно, используются ISR-варианты примитивов FreeRTOS.
